        ...
        $(window).ready(function (e) {
            ...
            function addRow(row) {
                ...
                for (var j= 0; j<row; j++) {
                    ...
                    html += '</select></td>' +
                            '<td class="td-select">' +
                            '<select name="warehouse_id_out" class="form-control">' +
                            '<option value="">请选择：</option>';
                    ...
                    html += '</select></td>' +
                            '<td class="td-select">' +
                            '<select name="warehouse_id_in" class="form-control">' +
                            '<option value="">请选择：</option>';
                    for (var i in warehouseList) {
                        html += '<option value="'+ warehouseList[i].id +'">'+ warehouseList[i].name +'</option>';
                    }
            ...
            function getTableData() {
                ...
                $('#salesList').find('tbody tr').each(function() {
                    ...
                    detail.warehouse_id_out = $(this).find('[name="warehouse_id_out"]').val();
                    detail.warehouse_id_in = $(this).find('[name="warehouse_id_in"]').val();
                    detail.code_list = $(this).find('[name="code_list"]').html();
                    detail.note = $(this).find('[name="note"]').html();
                    if (!detail.goods_id && !detail.num) {
                        return;
                    }
                    if (!detail.goods_id || !detail.num || !detail.warehouse_id_in || !detail.warehouse_id_out) {
                        $.alertMessager('商品、调出仓库、调入仓库、数量为必填参数', 'danger');
                        return false;
                    }
                    detailList.push(detail);
                });
                if (detailList.length == 0) {
                    $.alertMessager('请添加调拨的商品', 'danger');
                }
                return detailList;
            }
            ...
            $('#save').on('click', function() {
                ...
                $.post("<?php echo U('Stock/transfer');?>", {stockLog: stockLog, detailList: detailList}, function(data) {
                    console.log(data);
                    if (!data.status) {
                        ...
                        window.setTimeout(function() {
                            window.location = "<?php echo U('Stock/transferList');?>";
                        }, 2000);
                    }
                });
            })
            ...
            $('#salesList tbody').on('change', 'select', function() {
                ...
                var warehouse_id_out = $(this).parents('tr').find('select[name="warehouse_id_out"]').val();
