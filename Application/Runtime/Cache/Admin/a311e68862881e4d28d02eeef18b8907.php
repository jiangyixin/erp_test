...
<div class="clearfix full-container">
    ...
    <div id="main" class="col-xs-12 col-sm-10 main" style="overflow-y: scroll;">
        ...
        <ul class="breadcrumb">
            <li><i class="fa fa-map-marker"></i></li>
                <li class="text-muted">仓储管理</li><li class="text-muted">仓库列表</li>
        </ul>
                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-12 col-sm-9 button-list clearfix">
                            <div class="form-group">
                                <a title="新增" class="btn btn-primary" href="<?php echo U('Stock/warehouseInfo');?>">新增</a>&nbsp;
                                <a title="删除" target-form="ids" class="btn btn-danger ajax-post confirm" model="Link" href="<?php echo U('Stock/warehouseDel');?>">删除</a>&nbsp;
                            </div>
                        </div>
                        <!-- 搜索框 -->
                            ...
                            <div class="builder-table">
                                <div class="panel panel-default table-responsive">
                                    <table id="warehouseList" class="table table-bordered table-striped table-hover">
                                        ...
                                        <tbody>
                                        <?php if(is_array($warehouseList)): foreach($warehouseList as $key=>$vo): ?><tr title="" data-id="<?php echo ($vo["id"]); ?>">
                                                <td><input class="ids" type="checkbox" value="<?php echo ($vo["id"]); ?>" name="ids[]"></td>
                                                <td><a title="查看库存详情" href="<?php echo U('Stock/stockList');?>?id=<?php echo ($vo["id"]); ?>"><?php echo ($vo["name"]); ?></a></td>
...
<script type="text/javascript">
    ...
    var tableMenuData = [
        [
        {
            ...
            func: function() {
                ...
                window.location = "<?php echo U('Stock/stockList');?>?id=" + id;
            }
        },{
            text: "修改",
            func: function() {
                var id = $(this).attr('data-id');
                window.location = "<?php echo U('Stock/warehouseInfo');?>?id=" + id;
            }
        }, {
            text: "删除",
            func: function() {
                ...
                if (!result) return;
                var id = $(this).attr('data-id');
                $.post("<?php echo U('Stock/warehouseDel');?>", { id: id}, function(data) {
                    if (data.status) {
    ...
    $(window).ready(function(e) {
        ...
        $("#warehouseList tbody tr").smartMenu(tableMenuData, {
            name: "table"
        });
